...
$form.Text = 'Hadi WiFi Router - Portable Control Panel v2.2'
...
[void]$tabs.TabPages.AddRange(@($tabHot,$tabSchedule,$tabWeb,$tabSpeed,$tabDevices,$tabRemote,$tabDiag))
...
if($null -ne $c.PSObject.Properties['ScheduleRules'] -and @($c.ScheduleRules).Count -gt 0){
    ...
    # Migrate the old whole-traffic weekly schedule into the new rule grid.
